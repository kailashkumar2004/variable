// var day = "tuesday".toLowerCase();

const { questionInt } = require("readline-sync")

// if (day == "monday") {
//   console.log("Dhanwantari\nSiddhi\Kritika\Salon\nMayuri\nGauri");
// }
// else if (day == "tuesday") {
//   console.log("Keemaya\nRoshni\nPooja\Priyanka\nnalini\nManisha");
// }
// else if (day == "wednesday") {
//   console.log("Shrusti\Karuna\Parveen\Sandhya\nLeena\nKarishma");
// }
// else if (day == "thursday") {
//   console.log("Nikita\nMona\nPriyanka\Tejashree\nLovely\nMehzebin");
// }
// else if (day == "friday") {
//   console.log("Pihu\Sejal\nChaya\Shrestha\nMonali\nAardhangya");
// }
// else if (day == "saturday") {
//   console.log("Gunjan\nShweta\nRani\nIsha\nKusu\Madhu\n");
// }
// else if (day == "sunday") {
//   console.log("Swati\nKhusboo\nidhi\Preeti\nDipti\nAnamika");
// }
// else {
//   console.log("Please enter valid week day")
// }

// var x = 100;
// var y = 12
// var z = 90

 
// if (x > y){
//   if (x < z){
//       x = y
//   }
//   else{
//       m = z
//   }
// }
// else {
//   if(y > z){
//       m = yx
//   }
   
// }
// console.log(m)


// var time = 11;
// if (time >= 5 && time 
//   }
//   else { m = <= 6) {
//  console.log("sleep well");
// }
// else if (time >= 6 && time <= 7) {
//  console.log("Morning Exercise");
// }
// else if(time > 7 && time < 8.30){
//  console.log("Break+Breakfast");
// }
// else if ( time > 8.30 && time < 12){
//  console.log("Study");
// }
 

// let readlineSync = require("readline-sync");
// var number=readlineSync.questionInt("enter a number");
// if (number<10){
//    console.log("num is less than 10");
// }
// else if(number>10 && number<20){
//    console.log("num is less than 20");
// }
// else{
//    console.log("greater than 20");
// }


// var num1 = 300 
// let readlineSync = require("readline-sync");
// var num2=readlineSync.question("enter a number----");
// if(num1==num2){
//    console.log("equal hai")
// }
// else{
//    console.log("not equal")
// }

// var number1 = 44 
// let readlineSync = require("readline-sync");
// var number2=readlineSync.questionInt("enter a number---");
// if(number2>=number2){
//    console.log("number2 is greater than or equal");
// }
// else{
//    console.log("number2 is lesser");
// }
     
// let readlineSync = require("readline-sync");
// var water=readlineSync.questionInt("enter water level----");
// if (water<1){
//    console.log("more water needs to fill");
// }
// else if (water>=1 && water<=10){
//    console.log("no need to fill water");
// }
// else{
//    console.log("overflow");
// }

// let readlineSync = require("readline-sync");
// var varx=readlineSync.questionInt("enter a number---")
// if(varx%2===0){
//    console.log("divisible hai")
// }
// else{
//    console.log("not divisible")
// }

// let readlineaSync = require("readline-sync");
// var varX=readlineaSync.questionInt("enter a number-----");
// var vary=readlineaSync.questionInt("enter a number-----");
// if(varX%vary===0){
//    console.log("Divisible");
// }
// else{
//    console.log("Not divisible");
// }

// let readlineaSync = require("readline-sync");
// var number1=readlineaSync.questionInt("enter a number-----");
// var number2=readlineaSync.questionInt("enter a number----");
// if(number1>number2){
//    console.log("number1 is greater");
// }
// else{
//    console.log("number2 is greater");
// }


// let readlineaSync = require("readline-sync");
// var number=readlineaSync.questionInt("enter a number-----");
// if(number%5===0 && number%15===0){
//    console.log("Divisible by both");
// }
// else{
//    console.log("Not divisible by both");
// // }

// let readlineaSync = require("readline-sync");
// var age=readlineaSync.questionInt("enter age---");
// if(age>=5 && age<18){
//    console.log("You can go school");
// }
// if(age>=18 && age<21){
//    console.log("You can vote in elections.");
// }
// if(age>=21 && age<24){
//    console.log("You can drive a car");
// } 
// if(age>=24 && age<28){
//    console.log("You can marry");
// }
// if(age>=28 ){
//    console.log("You can drink");
// }


// let isTrue = "True";
// if(isTrue){
//   console.log("Output should show")
// }
// if(isTrue === "False"){
//   console.log("Both are same")
// }
// else {
//   console.log("Please run it. if all conditions false")
// }


// let a = 5;
// if(a){
//   console.log("Output should show")
// }
// if(a === "False"){
//   console.log("Both are same")
// }
// else {
//   console.log("Please run it. if all conditions false")
// }


// var number = 22
 
// if(number%3 == 0){
//    console.log("choco")
// }
// else if(number%7==0){
//    console.log("late")
// }
// else if(number%3==0 && number%7==0){
//    console.log("Chocolate")
// }
// else {
//    console.log("Not divisible by 3 , 7")
// }

// let readlineSync = require("readline-sync");
// var number=readlineSync.questionInt("enter a number-----");
// if (number<10){
//     console.log("num is less than 10");
// }
// else if(number>10 && number<20){
//     console.log("num is less than 20");
// }
// else{
//     console.log("greater than 20");
// }


// let readlineSync = require("readline-sync");
// var yourName= readlineSync.question("What is your name------")
// console.log(yourName);

// let readlineSync = require("readline-sync");
// var age= readlineSync.questionInt("What is your age:----- ")
// console.log(age);







// var day = "TUeSday".toLowerCase();

// if (day == "monday") {
//   console.log("Dhanwantari\nSiddhi\Kritika\Salon\nMayuri\nGauri");
// }
// else if (day == "tuesday") {
//   console.log("Keemaya\nRoshni\nPooja\Priyanka\nnalini\nManisha");
// }
// else if (day == "wednesday") {
//   console.log("Shrusti\Karuna\Parveen\Sandhya\nLeena\nKarishma");
// }
// else if (day == "thursday") {
//   console.log("Nikita\nMona\nPriyanka\Tejashree\nLovely\nMehzebin");
// }
// else if (day == "friday") {
//   console.log("Pihu\Sejal\nChaya\Shrestha\nMonali\nAardhangya");
// }
// else if (day == "saturday") {
//   console.log("Gunjan\nShweta\nRani\nIsha\nKusu\Madhu\n");
// }
// else if (day == "sunday") {
//   console.log("Swati\nKhusboo\nidhi\Preeti\nDipti\nAnamika");
// }
// else {
//   console.log("Please enter valid week day")
// }

// var readline =require("readline-sync")
// var day =readline.question("enter your any day=====")
// if(day=="sunday"){
//     console.log("holi day")
// }
// else if(day=="monday"){
//     console.log("working day")
// }
// else if(day=="tuesday"){
//     console.log("mothers day")
// }
// else if(day=="wenesday"){
//     console.log("brothers day")
// }
// else{
//     console.log("enter a valid day")
// }

// var day="monday"
// if(day=="sunday"){
//     console.log("enjoy day")
// }
// else if(day=="monday"){
//     console.log("hugday")
// }
// else{
//     console.log("enter valid day")
// }

// var readlineaSync =require("readline-sync")
// var time =readlineaSync.questionInt("enter your time=====")
// if(time>=9 && time<=10){
//     console.log("sellping")
// }
// else if(time>10 && time<11){
//     console.log("runing")
// }
// else if(time>=11 && time<=12){
//     console.log("enjoy")
// }
// else{
//     console.log("enter a valid time")
// }

// var time=10
// if(time<=10){
//     console.log("dance")
// }
// else if(time>=10){
//     console.log("masti")
// }
// else if(time=11){
//     console.log("singing")
// }
// else if(time<=12){
//     console.log("seleping")
// }
// else{
//     console.log("nothing")
// // }

// var a=11
// if(a%2==0){
//     console.log("even number")
// }
// else{
//     console.log("odd number")
// }
// var num=18
// if(num%2==0){
//     console.log("even num")
// }
// else {
//     console.log("odd num")
// }


// var readline =require("readline-sync")
// var num =readline.questionInt("enter any num==")
// if(num%2==0){
//     console.log("even number")
// }
// else{
//     console.log("odd num")
// }

// var readlineSync =require("readline-sync")
// let number =readlineSync.questionInt("enter any number===")
// if(number%2==0){
//     console.log("even number")
// }
// else{
//     console.log("odd number")
// }

// var num1=20
// var num2="18"
// if(num1<num2){
//     console.log("num1 se num2 bada hai ")
// }
// else if(num1>=num2){
//     console.log("num1 num2 se bada hai or equal hai")
// }
// else if(num1<num2){
//     console.log("num2 bada hai num1 se")
// }
// else{
//     console.log("sabhi condation glat hai")
// }

// var readlineaSync =require("readline-sync")
// var num =readlineaSync.questionInt("enter any number====")
// var num1 =readlineaSync.questionInt("enter the any num1===")
// if(num%2==0 && num1%5==0){
//     console.log("divisable hai")
// }
// else{
//     console.log("not divisble")
// }

// var name ="kailash"
// var a =name.length
// console.log(name.length)
// console.log(name)

// var village ="khosidangi"
// var x =village.length
// console.log(village.length)
// console.log(village)

// var readline =require("readline-sync")
// var name =readline.question('enter any name=====')
// if(name.length){
//     console.log("right")
// }
// else if(name){
//     console.log("wrong")
// }
// else{
//     console.log("nothing")
// }

// var a=20
// var b="20"
// console.log(a===b)

// var readline =require("readline-sync")
// var num =readline.questionInt("enter any num===")
// if(num<1){
//     console.log("khali daba")
// }
// else if(num>1 && num<5){
//     console.log("you can put more water")
// }
// else if(num>5 && num<10){
//     console.log("contener is full")
// }
// else {
//     console.log("water is over full")
// }

// var readline =require("readline-sync")
// var num =readline.questionInt("enter the any num====")
// var num1 =readline.questionInt("enter the any num1====")
// if(num>num1){
//     console.log( num,"bada hai")
// }
// else if(num==num1){
//     console.log("num num1 kaequal hai")
// }

// else{
//     console.log("num chota hai")

// }

// var num =100
// var num1=99
// if(num>num1){
//     console.log("bada hai")
// }
// else{
//     console.log("chota hai")
// }

// var readlineaSync =require("readline-sync")
// var num =readlineaSync.questionInt("enter the any num===")
// var num1 =readlineaSync.questionInt("enter the any num1==")
// if(num>num1){
//     console.log("bada hai")
// }
// else{
//     console.log("chota hai")
// }

// var a=44
// var b=200
// var c=a+b
// if(a==c){
//     console.log("yes")
// }
// else{
//     console.log("not")
// }

// var readline =require("readline-sync")
// var day =readline.question("enter a any day====")
// var time =readline.question("enter a any time=====")

// if(day=="monday" && time=="breakfast"){//|| day=="tuesday" || day=="wednesday"|| day=="friday"){
//     console.log("dal roti banegi aaj")
// }
// else if(day=="tuesday" && time=="diner"){
//     console.log("Pav-Bhaji banegi aaj toh")
// }
// else if(day=="wenesday" && time=="lunch"){
//     console.log("panir matar")
// }
// else if(day=="thueresday" && time=="evening breakfast"){
//     console.log("aalu dal ")
// }
// else if(day=="friday" && time=="morning breakfast"){
//     console.log("chiken rice")
// }
// else if(day=="saturday" && time=="evening lunch"){
//     console.log("matan rice")
// }
// else{
//     console.log("fix rice")
// }


// var value = "delhi"
// var guess = "Sheher ka naam guess karo"

// if(guess!=value){
// console.log("Aapka guess galat hai")
// }
// else{
//     console.log("Aapka guess sahi hai")
// }


// var readline =require("readline-sync")
// var day =readline.question("enter your any day====")
// var time =readline.question("enter the any time====")
// if(day=="monday"){
//     if(mela=="brakefast")
//     console.log("poha")
// }
// else if(day=="tuesday"){
//     if(mela=="breakfast")
//     console.log("poori sabji")
// }
// else{
// console.log("Aur kisi bhi din hum daal roti sabzi khaynege.")
// }


// var readline =require("readline-sync")
// var num =readline.questionInt("enter any num====")
// var num1 =readline.questionInt("enter the any num1====")
// if(num<10){
//     console.log("10 se chota hai")
// }
// else if(num>10 && num<20){
//     console.log("20 se chota hai")
// }
// else{
//     console.log("20 se bada hai")

// }

// var number = 300 
// var number1 = 123
// var varx =number-number1
// if(number==varx){
//     console.log("equal")
// }  
// else{
//     console.log("not equal")
// }

// var readline =require("readline-sync")
// var number =readline.questionInt("enter the any number===")
// // var number1 =readline.questionInt("enter the any number1===")
// var num =readline.questionInt("enter the any num===")
// if(number==num){
//     console.log("equal")
// }
// else{
//     console.log("not equai")
// }

// var readlineaSync =require("readline-sync")
// // var number =readlineaSync.questionInt("enter the any number===")
// var num1 =readlineaSync.questionInt("enter the any num1===")
// if(number>=num1){
//     console.log("equal to or grater")
// }
// else{
//     console.log("lesser")
// }

// var readline =require("readline-sync")
// var water =readline.questionInt("enter the any water====")
// if(water<1){
//     console.log("more water needs to be filled")
// }
// else if(water>1 && water<10){
//     console.log("no need to fill water")
// }
// else if(water>10){
//     console.log("overflow")
// }

// var readlineSync =require("readline-sync")
// var num =readlineSync.questionInt("enter the any num===")
// if(num%2==0){
//     console.log("divisble by 2")
// }
// else{
//     console.log("not divisble by 2")
// }

// varx =12
// if(varx%2==0){
//     console.log(varx, "is divisible by 2")
// }
// else{
//     console.log(varx,"is not divisible by 2")
// }

// varx =questionInt ("enter number1===== ") 
// vary = questionInt("enter number2==== ") 
// if(varx % vary == 0){ 
// console.log("Divisible")
// } 
// else{
// console.log("Not Divisible")
// } 

// var x=12
// var y=3
// if(x%y==0){
//     console.log("divisible")
// }
// else{
//     console.log("not divisible")
// }

// var readline =require("readline-sync")
// var num1 =readline.questionInt("enter the any num1====")
// var num2 =readline.questionInt("enter the any num2====")
// if(num1>num2==0){
//     console.log('num1')
// }
// else{
//     console.log("num2")
// }

// var readlineaSync =require("readline-sync")
// var num1 =readlineaSync.questionInt("enter the any num1====")
// var num2 =readlineaSync.questionInt("enter the any num2====")
// if(num1%5==0 && num2%10==0){
//     console.log("divisible")
// }
// else{
//     console.log("not divisible")
// }

// var readlineSync =require("readline-sync")
// var age =readlineSync.questionInt("enter the age===")
// if(age>=5 && age<=18){
//     console.log("can go to school")
// }
// else if(age>=18 && age<=21){
//     console.log("can vote in elections")
// }
// else if(age>=21 && age<=24){
//     console.log("can drive a car")
// }
// else {
//     console.log("can marry")
// }

// var number1=234
// var number2="1"
// console.log(number1+number2)

// var year = str(2016)
// var name = "NavGurukul"
// if(name + ', ' + year ){
//     console.log("mein str hua tha!")
// }

// var mangoes =5
// var mangoes =mangoes+5
// console.log(mangoes/5)

// var readline =require("readline-sync")
// var speed =readline.question("enter the any speed===")
// if(speed>60 && speed>30){
//     console.log("speed kam hai")
// }
// else if(speed>30){
//     console.log("speed bahut kam hai")
// }
// else if(speed>100){
//     console.log("speed bahut fast hai")
// }


// var readlineaSync =require("readline-sync")
// var speed =readlineaSync.questionInt("enter the any speed====")
// if(speed<60){
//     console.log("speed kam hai")
// }
// else if(speed<30){
// console.log("Speed bahot kam hai")
// }
// else if(speed>100){
//     console.log("Speed bahot fast hai")
// }

// var i = 10
// if(i>15){
// console.log("10 is less than 15")
// }
// else{
//     console.log("i am not in if")
// }

// var n=55
// if (n%5==0 && n%11==0){
// console.log("navgurukul")
// }
// else if (n%5==0){
// console.log("nav")
// }
// else if(n%11==0){
// console.log("guru")
// }
// else{
// console.log("not divisible 5 and 11")
// }

// var my_income=1000
// var my_expenditure=500
// if(my_expenditure>my_income){
//     console.log("You are currently in loss")
// }
// else{
//     console.log("You are in profit")
// }

// var a=19
// if(12>14){
//     console.log("mango")
// }
// else if(a==19){
//     console.log("papaya")
// }
// else if(a!=19){
//     console.log("grapes")
// }
// else{
//     console.log("apple")
// }

// var user=50
// if(user%2==0){
//     console.log("user-2,user-4,user-6")
// }
// else{
//     console.log("user+1,user+3,user+5")
// }

// var my_stream = "science"
// if (my_stream == "science"){
//     console.log("Welcome to a more scientific world")
// }
// else{
//     console.log("Welcome to the world")
// }

// var age=69
// if(age>50){
//     console.log("person is a senior citizen")
// }
// else{
//     console.log("person is not senior citizen")
// }


// var name= "maya"
// if(name=="saya"){
//     console.log("delh")
// }
// else if(name=="mana"){
//     console.log("mumbai")
// }
// else{
//     console.log("india")
// }

// var num1=100
// var num2=100
// if(num1==num2){
//     console.log("dono eqal hai")
// }
// else if(num1==num2 || num1>0){
//     console.log("equal hai or num1 bada hai")
// }
// else if(num1==num2 && num1<0){
//     console.log("dono equal hai num1 chota hai")
// }
// else if(num1==num2 && num1>0){
//     console.log("dono equal hai or num1 bada hai")
// }

// var nNum=1
// if(nNum!=0){
//     if(nNum>0){
//         console.log("Number is  positive  Grater")
//     }else{
//         console.log("Number is negative")
//     }
    
// }
// else{
//     console.log("Number is zero")
// }

// var x=3
// if(x>2 || x>3){
//     console.log("ok")
// }
// else{
//     console("no output")
// }


// var a=1
// var b=3
// var c=5
// if (a>=0){
//     if(b>3){
//         console.log("hi")
//     }else{
//     console.log("hyy")}
// }
// else if(c>3){
//     console.log("helo")
// }
// else{
// //     console.log("know program")
// }


